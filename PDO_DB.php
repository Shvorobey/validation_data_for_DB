    ...
    class PDO_DB
    {
        ...
        public static function update(array $data, $table, $id, $primary = 'id')
        {
            ...
            if (!empty($data)) {
                ...
                switch (self::getParams('type')) {
                    case 'pgsql':
                        $stm = self::prepare("UPDATE {$e_char}$table{$e_char} SET $str WHERE {$e_char}$primary{$e_char}=?",
                            [$id]);
                        break;

                    case 'mysql':
                    default:
                        $stm = self::prepare("UPDATE {$e_char}$table{$e_char} SET $str WHERE {$e_char}$primary{$e_char}=? LIMIT 1",
                            [$id]);
                }
        ...
        public static function row_by_hash($table, $hash, $primary = 'hash')
        {
            $e_char = self::getEscapeCharacter();

            $stm = self::prepare("SELECT * FROM {$e_char}$table{$e_char} WHERE {$e_char}$primary{$e_char}=? LIMIT 1",
                [$hash]);
            $record = $stm->fetch();
        ...
        public static function del_id($table, $id, $is_virtual = false, $del_column = 'is_del', $primary = 'id')
        {
            ...
            $query = $is_virtual ? "UPDATE $table SET {$e_char}$del_column{$e_char}=1 WHERE {$e_char}$primary{$e_char}=?" : "DELETE FROM $table WHERE {$e_char}$primary{$e_char}=?";
        ...
        public static function change_pos_from_to($table, $where, $posFrom, $posTo, $column = 'pos', $primary = 'id')
        {
            ...
            self::prepare("UPDATE {$e_char}$table{$e_char} SET {$e_char}$column{$e_char}=$posTo WHERE {$e_char}$primary{$e_char}=?",
                [$id]);
            return true;
        }
        ...
        public static function change_pos($table, $where, $id, $dir, $order = null, $column = 'pos', $primary = 'id')
        {
            ...
            switch ($dir) {
                case 'dup':
                    self::prepare("UPDATE {$e_char}$table{$e_char} SET {$e_char}$column{$e_char}=0 WHERE {$e_char}$primary{$e_char}=?",
                        [$id]);
                ...
                case 'ddown':
                    ...
                    self::prepare("UPDATE {$e_char}$table{$e_char} SET {$e_char}$column{$e_char}='$pos' WHERE {$e_char}$primary{$e_char}=?",
                        [$id]);
                    self::rebuild_pos($table, $where, "{$e_char}$column{$e_char}");
                    break;
                ...
                case 'up':
                    ...
                    if ($item2 !== false) {
                        self::prepare("UPDATE {$e_char}$table{$e_char} SET {$e_char}$column{$e_char}=? WHERE {$e_char}$primary{$e_char}=?",
                            [$pos2, $item1[$primary]]);
                        self::prepare("UPDATE {$e_char}$table{$e_char} SET {$e_char}$column{$e_char}=? WHERE {$e_char}$primary{$e_char}=?",
                            [$pos1, $item2[$primary]]);
                    }
        ...
        public static function reset_pos($table, $order = 'id', $column = 'pos')
        {
            ...
            for ($i = 0; $i < count($arr); $i++) {
                self::prepare("UPDATE {$e_char}$table{$e_char} SET {$e_char}$column{$e_char}=? WHERE id=?",
                    [++$c, $arr[$i]['id']]);
            }
        }
    }
